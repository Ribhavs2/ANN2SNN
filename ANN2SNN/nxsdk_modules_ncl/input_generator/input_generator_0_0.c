...
static uint32_t interval = 1024;

// Number of injection range address for the snip
// Needed to allocate static memory for RnageBasedStateAddress
static uint32_t numInjectionRangeAddress = 1;
static struct RangeBasedStateAddress injectionRangeAddress[1];
...
int doMgmt(runState *s) {
    ...
    if (s->time_step == 1){ 
        dataChannelId = getChannelID("dataChannel_0_0");
        infoChannelId = getChannelID("infoChannel_0_0");
        dataSizeChannelId = getChannelID("dataSizeChannel_0_0");
        metaDataChannelId = getChannelID("metaDataChannel_0_0");
...
void runMgmt(runState *s) {
    ...
    for (int i=0; i<numInjectionRangeAddress;i++){
        ...
        for(int j=0;j<injectionRangeAddress[i].length;j++){
            ...
            //          cxId, data[injectionsDone], 6, s->time_step);
            injectionsDone++;
